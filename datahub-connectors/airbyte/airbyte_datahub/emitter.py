...
from airbyte_datahub.config import PipelineConfig
...
def emit_all(config: PipelineConfig, dry_run: bool = False) -> None:
    ...
    all_mcps = (
        list(flow.generate_mcp())
        + [mcp for job in jobs for mcp in job.generate_mcp()]
    )
    for mcp in all_mcps:
        emitter.emit(mcp)
